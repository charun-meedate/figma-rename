...
import { findChains, loadMap } from './lib/map.mjs';
...
async function main() {
  ...
  if (!batchId) {
    ...
    for (const b of map.batches) console.error(`  ${b.id}  (${b.kind}, ${b.renames.length} rename(s))`);
    process.exit(1);
  }
  const batch = map.batches.find((b) => b.id === batchId);
  if (!batch) {
    throw new Error(`No batch called "${batchId}". Have: ${map.batches.map((b) => b.id).join(', ')}`);
  }
  ...
  const pairs = batch.renames.map((r) => ({
    id: r.id,
